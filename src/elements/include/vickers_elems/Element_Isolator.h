...
/**
  \file Element_Isolator.h Surrounds other Elements with itself and separates them spatially
...
#define ISOLATOR_VERSION 5
  ...
  public:
    ...
    virtual const char* GetDescription() const
    {
     ...
     {
       return Element<CC>::COMPLETE_DIFFUSABILITY ;
     }
	    ...
	    for (u32 i = md.GetFirstIndex(1); i <= md.GetLastIndex(R); ++i)
		 ...
		 { if (type == Element_Empty<CC>::THE_INSTANCE.GetType())
		   {  window.SetRelativeAtom(donutSite, self);  // copy itself to donut location
            ...
            // Look at all sites A of greater distance from this Isolator than the seen Atom;
            // if A has a non-empty non-isolator Atom in it, Isolator will try to swap the seen Element
            // one unit away from itself.
